...
from aisca_v2.core.attackmodels import AttackModel
from aisca_v2.core.predictmodels import MLP,CNN1D, target_scores_to_key_scores, _predict_key_scores


class MyEnsembleAttackModel(AttackModel):
    def __init__(self, owner_attack_instance, rnd, sbyte, for_encryption,
                 predict_models_intermediates=None, predict_models_post_processors=None):
        super().__init__(owner_attack_instance, rnd, sbyte, for_encryption,
                         predict_models_intermediates, predict_models_post_processors)

    @classmethod
    def get_predict_model_classes(cls):
        return [('MLP', MLP), ('CNN', CNN1D)]

    @property
    def mlp_model(self):
        return self.predict_models[0]
    ...
    def predict_key_scores(self, attack_datafile, attack_size, n_attacks, from_trace=0, real_key=None, **kwargs):
        ...
        scores_list = []
        for i in range(n):
            scores_list.append(_predict_key_scores(self.model_list[i], attack_datafile, attack_size, n_attacks, from_trace))
        total_key_scores = [(0,0) for _ in range(len(scores_list[0]))]
        for i in range(len(scores_list[0])):
            scores = 0
            for j in range(n):
                scores += scores_list[j][i][1]
            total_key_scores[i] = (scores_list[0][i][0],(scores)/n)

        return total_key_scores
